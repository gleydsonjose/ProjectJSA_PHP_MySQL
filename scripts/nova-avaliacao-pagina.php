<?php
    ...
    $nova_avaliacao_pagina = new Dados("nome do banco de dados", "host", "nome de usuario", "senha");

    // Buscando todas avaliações no banco de dados e passando para um array.
    $avaliacao_dados = $nova_avaliacao_pagina->BuscarAvaliacoes(); 

    foreach($avaliacao_dados as $ad){ 
        // Se o array id_avaliacoes não existir ou a avaliação no banco de dados não existir na página, será retornado a avaliação, sua modal e script ajax para a página.
        if(!isset($_POST["id_avaliacoes"]) || !in_array($ad["id"], $_POST["id_avaliacoes"])){
